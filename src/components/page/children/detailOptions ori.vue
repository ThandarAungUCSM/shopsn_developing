    ...
    <div class="detail-wrap" v-show="$store.state.const_join">
      <div class="content-main">
        <div class="padd">
          <div class="contet-item clearfix" v-if="$store.state.commodity_data">
            ...
            <p class="price" v-else>
              <span>{{$store.state.commodity_data.goods.integral}}�?�?</span>
              <span
                v-if="$store.state.commodity_data.goods.packaging"
              >{{$store.state.commodity_data.goods.packaging}}</span>
              <span v-else></span>
          ...
          <div class="content-scroll">
            ...
            > 
              <p>{{item.name}}</p>
              <!-- gray: emptyStockGoods.indexOf(Number(data.id)) != -1, -->
             <span
               :data-id="data.id"
               ...
               :class="{active:subIndex[index] == data.id}"
             >{{data.item}}{{subIndex}}</span>
              <!-- <span
                      :data-id="data.id"
                      :data-spec-id="data.spec_id"
                      v-if="item.id == data.spec_id"
                      v-for="data in spec.spec_children"
                      :class="{ activeTab: activeItem == data.item }"
                      :key="data.id"
                      @click="addClass(index,$event,data, data.id); sendValue(data.item, data.id)"
              >{{data.item}}</span> -->
            </div>

            <!-- 选择 长度 -->
            <div class="addSub" v-if="$store.state.commodity_data.goods.customized == 1">
              <span class="pull-left fl">长度(�?)</span>
            ...
            <div class="addSub">
              ...
              <div class="pull-right fr clearfix">
                <!--<span class="stock fl"-->
                <!--v-if="$store.state.commodity_data.goods.stock">库存：{{data.goods.stock}}�?</span>-->
                <div class="input-main fl clearfix">
            ...
            <div class="com">
              ...
              <div>
                数量�?
                <span>{{$store.state.commodity_data.goods.customized == 1 ? (Number($store.state.commodity_val) * rice).toFixed(2) : Number($store.state.commodity_val)}}</span>&nbsp;&nbsp; 共�??
        ...
        <div class="btn-join clearfix" v-if="$route.params.status == 1">
          <button class="fl imm" @click="toOrder">立即�?�?</button>
          <button class="join fl" @click="join">加入�?物车</button>
        </div>
        <div class="btn-join clearfix" v-if="$route.params.status == 2">
          <button class="fr imm" @click="remove">�?�?</button>
        </div>
      </div>
    </div>
  </transition>
