<template>
  <div class="product">
    ...
    <div class="selected" @click="theSon">
      ...
      <span style="float:right; padding-right: .2rem; font-size: .26rem;">{{spec}}</span>
      <span
              v-if="item"
...
<!--            </span>-->
          ...
          <div class="box_box">
            <div class="box_body" v-for="(item,index) in couponList" :key="index.id" @click="couponReceive(index)">
              <div class="body_left">
                ...
                <p class="left_time">有效�? {{new Date(item.use_start_time * 1000).getFullYear()+'.'+(new Date(Number(item.use_start_time * 1000)).getMonth()+1)+'.'+new Date(Number(item.use_start_time * 1000)).getDate()}} - {{new Date(item.use_end_time * 1000).getFullYear()+'.'+(new Date(Number(item.use_end_time * 1000)).getMonth()+1)+'.'+new Date(Number(item.use_end_time * 1000)).getDate()}}</p>
    ...
    <div class="prompt" @click="toTab">点击查看更�?�商品信�?</div>
    <detail-option
...
<script>
  ...
  import prList from "@/components/page/children/list.vue"; //猜你喜�?�和�?配�?��??
  ...
  import detailOption from "@/components/page/children/detailOptions.vue"; // 立即�?�? || 加入�?物车 弹�??
      ...
      matchGood(id) {
        this.axios
                ...
                .then(res => {
                  ...
                  console.log(this.$store.state.matchGood)
                })
                .catch(err => {
                  console.log(err);
                });
          ...
          params: {
            id: this.$route.params.id
                ...
                .then(res => {
                  ...
                  console.log('this.$store.state.commodity_data ' + this.$store.state.commodity_data)
